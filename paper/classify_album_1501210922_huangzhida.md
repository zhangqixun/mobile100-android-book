...
####  c. 故事相册归集
  ...
  综上所述，按时刻、旅行、故事归类，需要进行数据库表的设计，对相册与相片进行存储记录；然后通过android平台的MediaStore获取相片的信息, 主要是相片的exif信息，然后调用网络模块根据GPS信息获取相片的地址，最后调用底层C++库进行归集，相册归集完后通过回调的方式返回逻辑层，逻辑层再交付给UI层显示。底层的相册归集使用C++库的原因是为了实现跨平台，三大移动平台都可以通用，ios、android、wp，现在开发是两个版本ios和android版本。iOS平台的object-c可以直接调用C++, 而android平台则需要通过JNI调用。在实现这些功能的过程中，需要花费不少的时间在提高读图的效率，网络不通的图片地址的获取，UI的显示美化与设计，故事归集的准确度。目前为止，上述功能已经基本实现，在不断优化之中。
