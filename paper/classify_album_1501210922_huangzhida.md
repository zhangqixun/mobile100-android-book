...
#### 研究背景：由于系统自带的相册除了按时间归集之外，就没有其它类型的归集；但用户也想看到其他类型的归集，如旅行相册或故事相册等，或者是使用机器学习来识别图片是属于什么类型的相片。将图片归集成相册，会涉及
到Android的sqlite数据库表设计和操作，网络请求模块，JNI调用C++库接口，归集算法的设计与分析
